...
- `make setup`（= `bash scripts/setup.sh`）：一次安裝、將 bearer token 存入 Keychain、建立專用 allowed root `~/Downloads/telegram_mcp_files`，偵測內嵌 telegram-mcp 專案或 `TELEGRAM_MCP_SUMMARY_PROJECT` 後只額外授權其 `docs/chat/`，並把 Claude Code 與 Codex 設為連向同一個本機 HTTP server。
